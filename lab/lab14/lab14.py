...
def prune_min(t):
    ...
    if len(t.branches)==0:
        ...
        min_branches=[min_branch]
        prune_min(min_branch)
        t.branches=min_branches
...
def num_splits(s, d):
    ...
    "*** YOUR CODE HERE ***"
    def partitions(s):
        '''seperate the list s and return a two-diagram list of the partition'''
        if len(s)==0:
            return [[]]
        if len(s)==1:
            return [[s,[]]]
        last_partions=partitions(s[1:])[:]
        new_partitions=[]
        for partition in last_partions:
            new_partitions.append([partition[0]+[s[0]],partition[1]])
            new_partitions.append([partition[1]+[s[0]],partition[0]])
        return new_partitions
    partitions_s=partitions(s)
    #print(partitions_s)
    count=0
    for partition in partitions_s:
        if abs(sum(partition[0]) - sum(partition[1])) <= d:
            count += 1
    return count
...
class CheckingAccount(Account):
    ...
    def deposit_check(self,check):
        if check.payable_to==self.holder and check.count==0:
            check.count+=1
...
def reverse(lst):
    ...
    # def helper(lst,reversed_lst):
    #     if lst is Link.empty:
    #         return reversed_lst
    #     return helper(lst.rest,Link(lst.first,reversed_lst))
