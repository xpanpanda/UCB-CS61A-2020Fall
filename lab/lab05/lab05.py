...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    city_standard=make_city('city_standard',lat,lon)
    distance_a=distance(city_standard,city_a)
    distance_b=distance(city_standard,city_b)
...
def add_trees(t1, t2):
    ...
    add_branches=zip(branches(t1),branches(t2))
    add_label=label(t1)+label(t2)
    result_branches=[add_trees(b1,b2) for b1,b2 in add_branches]
    i=len(result_branches)
    result_branches+=branches(t1)[i:]+branches(t2)[i:]
    return tree(add_label,result_branches)
