...
def balanced(m):
    ...
    if is_planet(end(left(m))) and is_planet(end(right(m))):
        return w_left*l_left==w_right*l_right
    else:
        if w_left*l_left==w_right*l_right:
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t):
        if label(t)==find_value:
            ...
            return tree(result_label)
    else:
        return tree(result_label,[replace_leaf(b,find_value,replace_value) for b in branches(t)])
...
def multiple_references_explanation():
    return """Two resistor is isolated,but the first par makes them merged because of multiple references.The bound of par1 can be reached for the reason that different referrences reach the bound with different 
    registors' values"""
...
def quadratic(x, a, b, c):
    ...
    def func(t):
        return a*t*t+b*t+c
    if a==0:
        lower=min(func(i),func(j))
        upper=max(func(i),func(j))
    else:
        mid=-b/(2*a)
        if i<=mid<=j:
            lower=min(func(i),func(j),func(mid))
            upper=max(func(i),func(j),func(mid))
        else:
            lower=min(func(i),func(j))
            upper=max(func(i),func(j))
    return interval(lower,upper)
