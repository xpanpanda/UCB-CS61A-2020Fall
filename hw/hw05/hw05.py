class VendingMachine:
    ...
    def vend(self):
        ...
        if self.fund<self.price:
            ...
            self.num-=1
            self.flag=1
            return 'Here is your {1} and ${0} change.'.format(self.fund,self.name)
    ...
    def restock(self,number):
        self.num+=number
        return 'Current {1} stock: {0}'.format(self.num,self.name)
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if len(t.branches)>2:
            ...
            if not left.label<=t.label<=right.label:
                return False
            elif left.label>bst_min(right) or right.label<bst_max(left):
                return False
