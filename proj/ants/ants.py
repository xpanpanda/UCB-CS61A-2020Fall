...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.is_true:
            ...
            while self.place and current_place:
                if current_place.ant:
                    if not current_place.ant.is_doubled:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        if self.direction and self.place.entrance!=gamestate.beehive:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while current_place!=beehive:
            new_ant=current_place.ant
            new_bees=current_place.bees
            if new_ant:
                if new_ant is not self:
                    dic[new_ant]=distance
            if new_bees:
                for bee in new_bees:
                    dic[bee]=distance
            current_place=current_place.entrance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        new_damage=self.damage-self.insects_shot*0.05-distance*0.2
        if new_damage>0:
            return new_damage
        else:
            return 0
