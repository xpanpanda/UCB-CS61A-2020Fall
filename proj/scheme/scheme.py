...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        while current_args is not nil:
            python_args.append(current_args.first)
            current_args=current_args.rest
        if self.use_env:
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        #LambdaProcedure类中的self.env是它生成的环境
        # END PROBLEM 11
...
def make_let_frame(bindings, env):
    ...
    while curr_formal is not nil:
        env.make_child_frame(curr_formal.first,curr_formal.rest)
